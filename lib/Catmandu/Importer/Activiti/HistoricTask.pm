package Catmandu::Importer::Activiti::HistoricTask;
use Catmandu::Sane;
...
sub generator {
  ...
  sub {
    ...
    unless(@$results){
      ...
      my $res = $activiti->historic_task_instances(
        %$params,
        start => $start,
        size => $size
      )->parsed_content;
...
Catmandu::Importer::Activiti::HistoricTasks methods are not idempotent: Activiti feeds can only be read once.
